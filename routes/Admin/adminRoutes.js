...
// AdminRoutes.js (face-login section)
router.post("/face-login", async (req, res) => {
  try {
    ...
    for (let user of allUsers) {
      ...
      console.log("Comparing:", user.name, "Distance:", distance);
    ...
    // ================= Step 1: Check if user already has a session today =================
    ...
    // ✅ If session exists today AND not logged out, prevent login
    ...
    // ✅ If session exists today but logged out, prevent multiple daily login if needed
    if (todaySession && todaySession.logoutTime) {
      return res.json({
        success: true,
        alreadyLogged: true,
    ...
    // ================= Step 2: Create new login session =================
    const now = new Date();
    const newSession = {
      loginTime: now,
      pauseTime: [],
